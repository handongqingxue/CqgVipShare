				...
				for(var i=0;i<cfrList.length;i++){
					...
					var appendStr="<div class=\"item2_div\">";
						...
						if(cfr.scType==5)
							...
							appendStr+="元";
						appendStr+="</span>";
						appendStr+="<div class=\"confirmBut_div\" onclick=\"confirmCan('"+cfr.srUuid+"','"+cfr.scId+"','"+cfr.scType+"','"+cfr.shareMoney+"','"+cfr.fxzOpenId+"')\">确认取消</div>";
						appendStr+="</div>";
...
function confirmCan(srUuid,scId,scType,shareMoney,fxzOpenId){
	$.post("confirmCanShareVip",
		{srUuid:srUuid,scId:scId,scType:scType,shareMoney:shareMoney,fxzOpenId:fxzOpenId},
		function(data){
			if(data.status=="ok"){
				alert(data.message);
